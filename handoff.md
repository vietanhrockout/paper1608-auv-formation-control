...
## Phase C.0 Gate, Round 3 — Third GPT Audit Pass (`REVIEW_GPT_2026-08-16_R2.md`)

GPT reviewed round 2 (again committed directly to the repo by the user) and found round 2's resume fix, while a real improvement, still didn't prove what it needed to for a genuine multi-hour production crash. 3 P0 + 2 P1 findings, all independently re-verified before fixing — and this round's own testing process caught an ADDITIONAL real bug (a density discontinuity at the resume boundary) that none of the three review passes had explicitly named, which is itself evidence the "write a rigorous acceptance test, don't just eyeball plausibility" discipline is doing real work, not just theater.

1. **[P0, confirmed] C0b (round 2) never actually exercised the production wrapper.** `diagnose_stepC0b_checkpoint_resume_equivalence.m` (round 2) resumed by calling `projected_rk4_integrate` with `opts.resume` directly, using a checkpoint whose `.nsteps` reflected run B1's own shorter target ($t_{\text{mid}}=0.3$s, `nsteps=3000`) — not the eventual resume target ($t_{\text{final}}=0.6$s, `nsteps=6000`). Traced through `resume_projected_rk4_run.m`'s own logic: it would have REJECTED that exact checkpoint (`nsteps_total ~= checkpoint.nsteps`). So round 2's "PASS" proved the raw integrator's one-step-method continuation works, but said nothing about whether the actual production-facing wrapper functions correctly. **Fixed**: added `opts.max_steps` to `projected_rk4_integrate.m` (a controlled test hook — stops the loop early while `nsteps`/target-derived fields still reflect the TRUE original horizon, exactly as a real crash would leave them) and rewrote `diagnose_stepC0b_*` (v2) to launch a SAME-TARGET run, interrupt it via `max_steps`, discard its in-memory output (simulating the crash), and resume through `resume_projected_rk4_run.m` itself — the real wrapper, not the raw integrator.
2. **[P0, confirmed] A resumed run couldn't reconstruct the full figure history.** A checkpoint only stored the raw physics state `{t,X,k,...}` — the decimated `t_hot/X_hot` accumulated before the checkpoint lived only in the crashed process's memory and was never persisted, so `resume_projected_rk4_run.m` could only return the post-resume tail segment. A real 100s crash-and-resume would reach the correct final state but be unable to produce complete Figs. 2–9 over the full $[0,100]$s without recomputing the lost prefix — defeating the practical point of checkpointing for this phase. **Fixed**: checkpoints now also persist `t_hist_partial`/`X_hist_partial` (the decimated history accumulated so far in that call); `resume_projected_rk4_run.m` now stitches this with the newly-resumed segment (de-duplicating the shared boundary sample) and returns the FULL `[0,t_final]` history.
3. **[P0, confirmed] Resume didn't bind the checkpoint to its numerical/controller config.** The checkpoint saved neither `h`/`t_final`/`params`/`sat_cfg`/`cfg` nor any fingerprint of them; the only check (`ceil(t_final/h)==checkpoint.nsteps`) can't detect a different `(t_final,h)` pair with the same step count, or ANY change to `params`/`sat_cfg`/`cfg` (e.g. an accidentally different `inverse_lambda_eps`, reward mode, or actuator limit) — any of which would silently splice together a hybrid, scientifically invalid trajectory. **Fixed**: checkpoints now save `t_final_target`, `h`, `params`, `sat_cfg`, `cfg` verbatim; `resume_projected_rk4_run.m` hard-fails via `isequal()` comparison on ANY mismatch, naming exactly which field(s) differ, unless an explicit `force_mismatch=true` diagnostic override is passed (never for real production use).
4. **[P1, confirmed] Online actuator maxima did not carry forward across resume.** `max_tau_act_force`/`max_tau_act_moment` reset to 0 at the start of every `projected_rk4_integrate` call, including resumed ones — so a resumed run's final reported peak could miss a larger pre-checkpoint peak. **Fixed**: these are now saved in the checkpoint (when `track_actuator` was on) and seeded from `opts.resume` on continuation, symmetric with how `max_retraction`/`total_retracted` already worked.
5. **[P1, confirmed] P.1b v2 still only compared single max `tau_act` scalars, not the full trajectory**, while implicitly suggesting broader coverage. **Fixed**: `diagnose_stepP1b_epsilon_sensitivity.m` (v3) now computes and cross-compares full `tau_act` force/moment trajectories (not just one maximum per run) across $\epsilon$, and — per GPT's explicit request — reports **separate PASS/FAIL verdicts per metric group** (physical state / command / actuator / retraction) instead of one blended verdict, so "the physical trajectory is fine" and "the command/actuator channels are eps-sensitive" can each be cited precisely rather than collapsed into one number.

**Bonus finding, caught by the round-3 fix's OWN acceptance test, not named in either review**: the first working version of the round-3 resume fix passed the final-state/cumulative-stats checks but FAILED the full-history comparison — `resume_projected_rk4_run.m` was recomputing a fresh `store_stride` targeting ~1001 samples for the remaining segment, ignoring what stride the pre-crash run had actually been using, producing a stitched history with a density discontinuity right at the resume boundary (dense pre-crash samples, then abruptly 4x-sparser post-resume samples). **Fixed**: checkpoints now also save `store_stride`, and resume reuses it by default (override available via an explicit argument). Verified: rerunning the same acceptance test after the fix gives **exact equality** — `max|X_A - X_resumed|` across all 6001 shared timestamps = `0.000000e+00`, all cumulative stats (`nsteps`, `max_retraction`, `total_retracted`, `max_tau_act_force`, `max_tau_act_moment`) match exactly, and a negative test confirms the wrapper correctly REJECTS a mismatched-target resume attempt.

**Phase C.0 Gate, Round 3: all 5 items + the self-caught bonus bug fixed and independently verified.**

### P.1b v3 final result (per-metric-group verdict)

```
PHYSICAL STATE (E_chi, E_s):     PASS  (rel spread 2.1e-7 / 3.2e-7)
COMMAND (tau_cmd, unsaturated):  FAIL  (rel spread 5.42%)
ACTUATOR (tau_act force/moment): FAIL  (force: 0% -- always saturated at 150N regardless of eps;
                                         moment: 33.0% rel spread, 21.2->25.7 Nm across eps)
RETRACTION (max_retraction):     FAIL  (17.3% rel spread: 3.92e4 -> 3.91e4 -> 3.82e4 -> 3.24e4,
                                         monotonically DECREASING as eps increases -- mechanistically
                                         sensible: larger eps softens the near-v=0 singularity, so
                                         smaller virtual_accel/NN-update spikes, so smaller max
                                         single-step retraction correction. total_retracted, the
                                         COUNT of retraction events, stays essentially flat across
                                         eps at <0.04% spread -- retractions happen just as often,
                                         just with somewhat smaller peak magnitude for larger eps.)
OVERALL: FAIL
```

Splitting the verdict into groups (per GPT's round-3 request) makes the finding much clearer than a single blended number: **the claim this project actually needs — that Issue O/P's convergence and Figs. 2/3/6/7/8/9 are not eps-sensitive — is solidly PASS.** The command/actuator/retraction sensitivity is real, mechanistically understood, and now precisely quantified (not just "some FAIL somewhere"), and is exactly the kind of caveat that belongs in a final report's assumptions section, not a blocker for the physical-state claims Phase C is actually being run to support. Log: `p1b_console.txt`, `p1b_stdout.txt`; structured verdict: `p1b_result.mat`.

### Phase C scope guidance (GPT, round 3 review)
Per GPT's explicit interpretation guidance (not independently re-derived, but adopted as reasonable and consistent with the evidence above): the P.1b FAIL does not block a 100s run at the fixed, documented production value `inverse_lambda_eps=1e-6`, because the tested physical error trajectories differ only at ~1e-7 relative scale. It DOES block any claim that the regularization is globally non-load-bearing for command-level or RL-cost quantities. Therefore: **physical Figs. 2, 3, 6, 7, 8, 9 may proceed** once the resume infrastructure above is trusted; **Figs. 4 and 5 remain provisional** for both the known critic-projection pathology (Issue M/K) and the newly-quantified epsilon sensitivity; **the eventual final report must identify epsilon as a documented numerical-implementation choice** and report the `tau_cmd` 5.4% trajectory-level relative spread, the `tau_act` moment-channel **33.0%** trajectory-level relative spread (P.1b v3's actual committed metric) and separately the raw **21.2→25.7 Nm** run-level maximum range (a different quantity, not a percentage — round-4 audit caught and fixed an earlier wording conflation of these two), and the 17.3% `max_retraction` spread, all explicitly, not silently.

## Phase C.0 Gate, Round 4 — Fourth GPT Audit Pass (`REVIEW_GPT_2026-08-17_R3.md`)

GPT reviewed round 3's resume fix and found it closes the SINGLE-interruption acceptance path convincingly (production wrapper exercised, config mismatches rejected, bit-exact stitched history, actuator maxima preserved), but identified two remaining restart-integrity gaps before calling the recovery path robust for an actual multi-hour Phase C run, plus two smaller wording issues. All 4 independently re-verified before fixing.

1. **[P0, confirmed] A second crash after resume would lose the original history prefix.** Traced the exact failure sequence GPT described: a resumed call's `t_hot`/`X_hot` started fresh at `resume.t`, so if THAT call wrote its own checkpoint, the checkpoint's `t_hist_partial` only covered the new segment — a second crash, resumed from that second checkpoint, would silently return `[t1,t_final]` with the original `[0,t1]` prefix gone. Round 2/3's `diagnose_stepC0b_*` only ever simulated a SINGLE interruption, so this could not have been caught by the existing test suite. **Fixed**: a resumed call now seeds its own `t_hot`/`X_hot` arrays with whatever prefix `opts.resume.t_hist_partial`/`X_hist_partial` carries, so every checkpoint (and the final return value) always contains the complete history back to `t=0`, no matter how many times a run has been interrupted and resumed. `resume_projected_rk4_run.m` no longer does its own post-hoc stitching (would now double-prepend) — it returns the integrator's own output directly. **Verified** by a new `paper1608/verify/diagnose_stepC0c_multi_resume_equivalence.m`, implementing GPT's exact requested acceptance test: original launch → simulated crash #1 → checkpoint C1 → resume #1 through the production wrapper → simulated crash #2 → checkpoint C2 → resume #2 through the production wrapper → completion. **Result: bit-exact** — `max|X_A - X_resumed_twice|` across all 9001 shared timestamps (a 0.9s test horizon) = `0.000000e+00`, all cumulative stats (`nsteps`, `max_retraction`, `total_retracted`, `max_tau_act_force`, `max_tau_act_moment`) identical to an uninterrupted baseline.
2. **[P0, confirmed] Checkpoints were not bound to the source-code version.** Round 2 explicitly asked for a "code/version commit" fingerprint; round 3 only added config-struct binding (`t_final`/`h`/`params`/`sat_cfg`/`cfg`), missing the actual source-code state — if the `.m` files changed between crash and resume while every config struct stayed byte-identical, resume would silently continue with a different vector field/integrator. **Fixed**: new shared utility `paper1608/simulation/git_fingerprint.m` (used by both the integrator and the resume wrapper, avoiding logic duplication/drift) records the git HEAD commit SHA and working-tree dirty state in every checkpoint. `resume_projected_rk4_run.m` hard-fails on a SHA mismatch (added to the same mismatch-collection/`force_mismatch`-override mechanism as the config checks) and warns if the checkpoint was itself written from a dirty tree. `exp4_rl_pts_mc_projected.m` (the actual production entry point) now refuses to LAUNCH a fresh checkpointed run from a dirty tree by default (`allow_dirty_launch=true` override for deliberate diagnostic/dev use, per GPT's "prefer refusing production launch from a dirty tree"). **Verified**: both C0b and C0c reruns correctly emit the new dirty-tree warning (the repo is legitimately dirty mid-development right now) and the mismatch-rejection negative test still passes.
3. **[P1, confirmed] The "uniform density" claim in `projected_rk4_integrate.m`'s docstring was stronger than the actual guarantee.** Reusing `store_stride` across a resume (round 3's fix) prevents the ~4x density-jump bug, but `resume.t` is always stored as the first sample of a resumed segment regardless of whether it falls exactly on a `step mod store_stride==0` boundary — so one extra off-grid sample is possible at each resume boundary. Harmless for physics/plotting (a genuine, correctly-computed state, just not evenly spaced from its neighbors), but the docstring overclaimed strict uniformity. **Fixed**: corrected the docstring to state precisely what round 3's fix does and does not guarantee.
4. **[P2, confirmed] Epsilon-sensitivity wording conflated two different quantities.** `IMPLEMENTATION_STATUS.md`'s "Known discrepancies" section still said "up-to-21%" (a round-2-era, run-level-maximum framing) in the same breath as P.1b v3's actual committed metric (33.0% trajectory-level relative spread) — two different normalizations of different things, never reconciled after P.1b was upgraded to v3. **Fixed**: now states both explicitly and separately (`tau_cmd` 5.4% trajectory-level spread; `tau_act` moment-channel 33.0% trajectory-level spread; and separately the raw 21.2→25.7 Nm run-level maximum range, not expressed as a percentage at all).

**Phase C.0 Gate, Round 4: all 4 items fixed and independently verified**, including a genuinely harder acceptance test (two chained interruptions, both resumed through the actual production wrapper) than any prior round attempted. Given the pattern across all four rounds — each fix survived its own round's test but a subsequent, more adversarial test found a real remaining gap — the next reviewer should not treat this round's "all verified" as license to skip re-checking; the fixes are real and each is backed by a passing bit-exact test, but the *pattern itself* (single-interruption tests undercounting multi-interruption reality) is worth remembering as a class of gap to watch for elsewhere in this project too.
