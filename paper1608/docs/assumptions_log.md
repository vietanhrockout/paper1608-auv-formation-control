...
* **Implementation assumption (SUPERSEDED 2026-08-18)**: originally fed the pre-saturation command $\tau_i = \tau_{\text{cmd}, i}$ into `strategic_utility.m` and `critic_update.m`. **This is no longer what the code does.**
...
* **Status**: **RESOLVED / SUPERSEDED 2026-08-18.** The project owner's academic supervisor determined that Eq. (16)'s $\tau$ is the physically applied, actuator-saturated input $\tau_{\text{act}}$ (Eq. 2/3's practical input). Production default is now `params.critic_reward_tau_mode = 'tau_act_saturated'`; `'tau_cmd_raw'` remains an opt-in comparison mode. Empirically this also eliminated the critic projection thrashing on the production integrator (`total_retracted` 641,485 -> 0 over 1e6 steps). The earlier VERIFIED status referred only to the literal-notation reading being implemented faithfully, not to it being the correct interpretation.
