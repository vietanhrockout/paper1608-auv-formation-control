function fig = plot_fig5_actor_nn_output(rl, manifest, params)
    ...
    caveat = sprintf(['PROVISIONAL (Issue M/K). LEFT: RBF activations, range (0,1] by construction -- fits the paper''s [0,1.5] axis; they freeze once chi/upsilon converge (constant RBF input). ' ...
        'RIGHT: actor network output f_RL, observed max|f_RL|=%.2f -- would not fit [0,1.5], and DRIFTS LINEARLY rather than settling (Wa still integrating; ||Wa||_F=16.86 is under delta_a=50 here but would reach it on a longer horizon). ' ...
