...
diagnostics_dir = fullfile(repo_root, 'artifacts', 'diagnostics');
...
try
    ...
    fid = fopen(fullfile(paths.diagnostics, 'test_out.txt'), 'w');
    ...
    fid = fopen(fullfile(diagnostics_dir, 'err.txt'), 'w');
    fprintf(fid, 'Error: %s\n', ME.message);
